...
export default function CustomerManagement() {
  ...
  const [reg, setReg] = useState<Omit<Customer, "id">>({
    username: "", password: "", name: "", email: "", phone: "+94 ", address: "", type: "REGULAR"
  });
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    alert(`✓ Registered! Customer ID: ${next.id}`);
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const found = customers.find(c => c.username === loginUser && c.password === loginPass);
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!loggedIn) return;
    const next = customers.map(c => c.id === loggedIn.id
      ? { ...c, email: pEmail || c.email, phone: pPhone || c.phone, address: pAddress || c.address }
      : c);
    setCustomers(next);
    setLoggedIn({ ...loggedIn, email: pEmail || loggedIn.email, phone: pPhone || loggedIn.phone, address: pAddress || loggedIn.address });
  ...
  const upgradeVIP = (id: number) => {
    ...
    alert(`✓ Customer #${id} upgraded to VIP.`);
  };
            ...
            {filtered.map(c => (
              <article key={c.id} className="cust-card">
                ...
                <div className="cust-discount-info">
                  Discount: {c.type === "VIP" ? "15%" : "5%"} on bookings
                </div>
      ...
      {tab === "profile" && (
        loggedIn ? (
          <form className="cust-form" onSubmit={handleProfileSave}>
            ...
            <div className="cust-discount-info">
              {loggedIn.type === "VIP" ? "★ VIP Customer · 15% discount" : "Regular Customer · 5% discount"}
            </div>
            ...
            <button className="cust-form__btn" type="button" style={{ background: "#333", color: "#fff", marginTop: "0.5rem" }} onClick={() => { setLoggedIn(null); setTab("login"); }}>Logout</button>
          </form>
        ) : (
