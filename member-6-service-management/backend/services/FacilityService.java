...
/** Service & Facility Management Service. Author: Member 6 */
public class FacilityService {
    ...
    private CustomerService parseCustomerService(String line) {
        ...
        try {
            return new CustomerService(
                    ...
                    Integer.parseInt(p[3]), Integer.parseInt(p[4]), Double.parseDouble(p[5]), p[6]);
        } catch (Exception e) { return null; }
    }
    ...
    public boolean addService(Service s) {
        ...
        services.add(s); persistServices(); return true;
    }
    ...
    public CustomerService requestService(int customerId, int reservationId, int serviceId, int quantity) {
        ...
        CustomerService cs = new CustomerService(nextCustomerServiceId++, customerId, reservationId, serviceId, quantity, total, "REQUESTED");
        customerServices.add(cs); persistCustomerServices(); return cs;
    }

    public List<CustomerService> getCustomerServicesByReservation(int reservationId) {
        List<CustomerService> list = new ArrayList<>();
        for (CustomerService cs : customerServices) if (cs.getReservationId() == reservationId) list.add(cs);
        return list;
    }

    public List<CustomerService> getAllCustomerServices() { return new ArrayList<>(customerServices); }
    ...
    public boolean cancelCustomerService(int id) { return updateCustomerServiceStatus(id, "CANCELLED"); }
