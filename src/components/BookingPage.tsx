...
export default function BookingPage() {
  ...
  const validateEmail = (email: string) => {
    ...
    if (email.split("@")[0].length === 0) return "Please enter a valid email address";
    if (email.endsWith("@")) return "Please enter a valid email address";
    return "";
  };

  const validatePhone = (phone: string) => {
    const digits = phone.replace(/\D/g, "");
    if (!phone.replace(/\s/g, "")) return "Phone number is required";
    if (digits.length < 9) return "Please enter a valid phone number";
    return "";
  };

  const validateCardNum = (num: string) => {
  ...
  const validateField = (name: string, value: any) => {
    ...
    switch (name) {
      ...
      case "cardName": if (!value) error = "Cardholder name is required"; break;
      case "cardNum": error = validateCardNum(value); break;
      case "cardExp": error = validateCardExp(value); break;
      case "cardCvv": error = validateCardCvv(value); break;
    }
  ...
  const handleBlur = (name: string) => {
    ...
    const values: Record<string, any> = { firstName, lastName, email, phone, cardName, cardNum, cardExp, cardCvv };
  ...
  const canProceed = (() => {
    ...
    if (step === 3) {
      ...
      const baseFields = firstName && lastName && !emailErr && !phoneErr && agree;

      if (!baseFields) return false;

      if (payMethod === "card") {
        return cardName && !validateCardNum(cardNum) && !validateCardExp(cardExp) && !validateCardCvv(cardCvv);
      }
      return true;
    }
    return true;
  ...
  const handleConfirm = () => {
    ...
    const refNumber = `AUR-${Date.now().toString().slice(-6)}`;

    // 1. UPDATE ROOMS AND STATUS IN LOCALSTORAGE
    const rawRooms = localStorage.getItem("aurum-rooms");
    let currentRooms = [];
    if (rawRooms) {
      try { currentRooms = JSON.parse(rawRooms); } catch { }
    } else {
      currentRooms = [
    ...
    // Map selected room and find matching room option in our active admin inventory
    const matchingRoom = currentRooms.find((r: any) => r.type === selectedRoom.name && r.status === "Available")
      || currentRooms.find((r: any) => r.type === selectedRoom.name); // fallback to first matching type if none are available

    const assignedRoomId = matchingRoom ? String(matchingRoom.id) : "102";

    if (matchingRoom) {
      matchingRoom.status = "Occupied";
    }

    // 2. UPDATE RESERVATIONS IN LOCALSTORAGE
    const rawReservations = localStorage.getItem("aurum-reservations");
    let currentReservations = [];
    if (rawReservations) {
      try { currentReservations = JSON.parse(rawReservations); } catch { }
    } else {
      currentReservations = [
    ...
    const newReservation = {
      ...
      checkIn: checkIn,
      checkOut: checkOut,
      status: payMethod === "cash" ? "Pending" as const : "Confirmed" as const
    };

    currentReservations.push(newReservation);

    // 3. UPDATE CUSTOMERS IN LOCALSTORAGE
    ...
    const existingCustomer = currentCustomers.find((c: any) => c.email.toLowerCase() === email.toLowerCase());
    if (existingCustomer) {
      existingCustomer.stays += 1;
    } else {
    ...
    // 4. UPDATE PAYMENTS IN LOCALSTORAGE
    const rawPayments = localStorage.getItem("aurum-payments");
    let currentPayments = [];
    if (rawPayments) {
      try { currentPayments = JSON.parse(rawPayments); } catch { }
    } else {
      currentPayments = [
    ...
    const paymentMethodLabel = payMethod === "card" ? "Card" : payMethod === "cash" ? "Pay at Hotel" : "Bank Transfer";
    const paymentStatus = payMethod === "cash" ? "Pending" as const : "Paid" as const;
    ...
    // 5. UPDATE SERVICES IN LOCALSTORAGE (IF ADD-ONS CHOSEN)
    let currentServices = [];
    if (extras.size > 0) {
      const rawServices = localStorage.getItem("aurum-services");
      if (rawServices) {
        try { currentServices = JSON.parse(rawServices); } catch { }
      } else {
        currentServices = [
      ...
      extras.forEach((extraId) => {
        let serviceId = "";
        if (extraId === "breakfast") serviceId = "S-03";
        else if (extraId === "airport") serviceId = "S-02";
        else if (extraId === "spa") serviceId = "S-01";

        if (serviceId) {
          const service = currentServices.find((s: any) => s.id === serviceId);
          if (service) {
            service.requests += 1;
          }
        }
    ...
    // Persist all updates to the backend text-file database in a single atomic transaction
    (async () => {
      const allData = {
        rooms: currentRooms,
        ...
        services: extras.size > 0 ? currentServices : JSON.parse(localStorage.getItem("aurum-services") || "[]"),
        reviews: JSON.parse(localStorage.getItem("aurum-reviews") || "[]"),
        authUsers: JSON.parse(localStorage.getItem("aurum-auth-users") || "[]")
      };

      // Update localStorage for all collections
      localStorage.setItem("aurum-rooms", JSON.stringify(currentRooms));
      localStorage.setItem("aurum-reservations", JSON.stringify(currentReservations));
      localStorage.setItem("aurum-customers", JSON.stringify(currentCustomers));
      localStorage.setItem("aurum-payments", JSON.stringify(currentPayments));
      if (extras.size > 0) {
        localStorage.setItem("aurum-services", JSON.stringify(currentServices));
      }

      // Final save to server .txt files
      const success = await apiClient.saveDb(allData);
      if (!success) {
        console.error("Failed to sync data to server files.");
      }
    })();

    // 6. TRACK GUEST SESSION HISTORY ACTIVITIES
    trackActivity(
      "booking",
      "Booking Confirmed",
      `Successfully reserved a ${selectedRoom.name} for ${nights} night(s), ${rooms} room(s). Reference: ${refNumber}`,
      { room: selectedRoom.name, nights, rooms, reference: refNumber }
    );

    trackActivity(
      "payment",
      "Payment Processed",
      `Processed payment of ${formatLKR(total)} via ${paymentMethodLabel}.`,
      { amount: formatLKR(total), method: paymentMethodLabel, status: paymentStatus }
    );

    trackActivity(
      "view-room",
      "Room Booked",
      `Booked and checked out the details for the luxury ${selectedRoom.name}.`,
      { room: selectedRoom.name, type: selectedRoom.type }
    );

    addToast(
      `🎉 Booking confirmed! Reference #${refNumber}. Confirmation sent to ${email}.`,
      "success"
    );
                ...
                {STEPS.map((label, i) => {
                  ...
                  return (
                    <div key={label} className="flex items-center flex-1 gap-3">
                      <div
                        className={`relative shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${active
                      ...
                      <div className={`text-xs sm:text-sm uppercase tracking-widest font-medium hidden sm:block ${active ? "text-gold-400" : done ? "text-gray-300" : "text-gray-600"
                        }`}>
                        {label}
                      </div>
                      {i < STEPS.length - 1 && (
                  ...
                  {roomOptions.map((r) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left bg-dark-300 border-2 transition-all duration-300 rounded-sm overflow-hidden grid sm:grid-cols-[200px_1fr] ${selected
                  ...
                  {addOns.map((a) => {
                    ...
                    return (
                      <button
                        ...
                        className={`text-left bg-dark-300 border-2 p-5 rounded-sm transition-all duration-300 ${selected
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <div className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-all ${selected
                ...
                <div className="bg-dark-300 border border-white/10 rounded-sm p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {([
                      ...
                      <button
                        ...
                        className={`p-4 border-2 rounded-sm text-center transition-all ${payMethod === m.id
                  ...
                  {payMethod === "card" && (
                    <div className="space-y-4 animate-[fadeIn_0.3s_ease-out]">
                      <Field label="Cardholder name *" error={touched.cardName ? errors.cardName : ""}>
                        <Input
                          value={cardName}
                          onChange={(v) => { setCardName(v); validateField("cardName", v); }}
                          onBlur={() => handleBlur("cardName")}
                          placeholder="JOHN DOE"
                          hasError={touched.cardName && !!errors.cardName}
                        />
                      </Field>
                      <Field label="Card number *" error={touched.cardNum ? errors.cardNum : ""}>
                        <Input
                          value={cardNum}
                          onChange={(v) => {
                            const val = v.replace(/\D/g, "").slice(0, 16);
                            setCardNum(val);
                            validateField("cardNum", val);
                          }}
                          onBlur={() => handleBlur("cardNum")}
                          placeholder="1234 5678 9012 3456"
                          hasError={touched.cardNum && !!errors.cardNum}
                        />
                      </Field>
                      <div className="grid grid-cols-2 gap-4">
                        <Field label="Expiry *" error={touched.cardExp ? errors.cardExp : ""}>
                          <Input
                            value={cardExp}
                            onChange={(v) => {
                              let val = v.replace(/[^\d/]/g, "").slice(0, 5);
                              if (val.length === 2 && !val.includes("/") && v.length > cardExp.length) {
                                val += "/";
                              }
                              setCardExp(val);
                              validateField("cardExp", val);
                            }}
                            onBlur={() => handleBlur("cardExp")}
                            placeholder="MM/YY"
                            hasError={touched.cardExp && !!errors.cardExp}
                          />
                        </Field>
                        <Field label="CVV *" error={touched.cardCvv ? errors.cardCvv : ""}>
                          <Input
                            value={cardCvv}
                            onChange={(v) => {
                            ...
                            placeholder="123"
                            hasError={touched.cardCvv && !!errors.cardCvv}
                          />
                        </Field>
                      </div>
                    </div>
...
function Field({ label, children, error }: { label: string; children: React.ReactNode; error?: string }) {
  return (
    <div className="space-y-1">
      ...
      {children}
      {error && (
        <p className="text-[10px] text-red-500 uppercase tracking-wider font-bold animate-[slideIn_0.2s_ease-out]">
          {error}
        </p>
      )}
    </div>
...
function Input(props: {
  ...
  return (
    <input
      ...
      className={`w-full bg-dark-400 border px-4 py-3.5 text-white placeholder-gray-600 outline-none transition-all rounded-sm ${props.hasError
