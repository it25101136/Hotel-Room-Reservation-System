...
export default function Reviews() {
  const [reviews, setReviews] = useState<Review[]>(() => {
    const raw = localStorage.getItem("aurum-public-reviews");
    if (raw) {
      try { return JSON.parse(raw); } catch {}
    }
    localStorage.setItem("aurum-public-reviews", JSON.stringify(initialReviews));
    ...
    async function loadPublicReviews() {
      ...
      if (db && db.reviews) {
        const approvedReviews = db.reviews
          ...
          .map((r: any) => ({
            id: parseInt(r.id.replace("RV-", ""), 10) || Date.now(),
            name: r.guest,
            rating: r.rating,
            title: r.comment.includes(": ") ? r.comment.split(": ")[0] : "Feedback",
            text: r.comment.includes(": ") ? r.comment.split(": ").slice(1).join(": ") : r.comment,
            date: new Date().toISOString().split("T")[0],
            verified: true
          }));
        setReviews(approvedReviews);
        localStorage.setItem("aurum-public-reviews", JSON.stringify(approvedReviews));
      }
    }
    loadPublicReviews();
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Update public reviews locally
    const updatedPublicReviews = [newPublicReview, ...reviews];
    setReviews(updatedPublicReviews);
    localStorage.setItem("aurum-public-reviews", JSON.stringify(updatedPublicReviews));

    // Also push to the admin workspace reviews list (aurum-reviews) under "Pending" status
    const rawAdminReviews = localStorage.getItem("aurum-reviews");
    let currentAdminReviews = [];
    if (rawAdminReviews) {
      try { currentAdminReviews = JSON.parse(rawAdminReviews); } catch {}
    } else {
      currentAdminReviews = [
    ...
    const newAdminReview = {
      id: `RV-${newReviewId.toString().slice(-4)}`,
      guest: formData.name,
      rating: newRating,
      room: "102",
      status: "Pending" as const,
      comment: `${formData.title}: ${formData.text}`
    };

    currentAdminReviews.unshift(newAdminReview);
    
    // Save new pending review to the text file database
    apiClient.syncCollection("reviews", currentAdminReviews);

    // Track review activity in guest's session context
    trackActivity(
      "review",
      "Submitted Review",
      `Posted a ${newRating}-star review: "${formData.title}"`,
      { rating: newRating, title: formData.title }
    );

    setFormData({ name: "", title: "", text: "" });
    setNewRating(0);
    setShowForm(false);
    addToast("Thank you! Your review has been submitted for moderation.", "success");
  };
