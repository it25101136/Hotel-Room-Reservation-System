...
import models.CustomerServiceRequest;
 ...
 * Manages services in services.txt
 * Manages customer-selected services in customer_services.txt
 */
public class FacilityService {
    ...
    private final List<CustomerServiceRequest> customerServices = new ArrayList<>();
    ...
    private void seedDefaultServices() {
        // Match frontend spa treatments
        addService(new PremiumService(nextServiceId++, "Royal Gold Facial",   105000, true, "90 min luxury facial",     "SPA"));
        addService(new PremiumService(nextServiceId++, "Deep Tissue Massage",  72000, true, "60 min therapeutic massage","SPA"));
        addService(new PremiumService(nextServiceId++, "Aromatherapy Journey", 92000, true, "75 min aromatherapy",       "SPA"));
        addService(new PremiumService(nextServiceId++, "Hot Stone Therapy",   115000, true, "90 min hot stone therapy",  "SPA"));
        // Basic services
        addService(new BasicService(nextServiceId++,   "Wi-Fi (Premium)",       2500, true, "High-speed unlimited Wi-Fi","ROOM"));
        addService(new BasicService(nextServiceId++,   "Breakfast Buffet",     12000, true, "Continental + Sri Lankan",  "DINING"));
        addService(new BasicService(nextServiceId++,   "Airport Pickup",       18000, true, "BIA Colombo → Hotel",      "TRANSPORT"));
        addService(new BasicService(nextServiceId++,   "Valet Parking",         5000, true, "24/7 secured parking",      "TRANSPORT"));
    }

    // ===== SERVICES file =====
    private void loadServices() {
        List<String> lines = FileHandler.readAllLines(SERVICES_FILE);
    ...
    // ===== CUSTOMER SERVICES file =====
    private void loadCustomerServices() {
        List<String> lines = FileHandler.readAllLines(CUSTOMER_SERVICES_FILE);
        for (String line : lines) {
            CustomerServiceRequest cs = parseCustomerService(line);
            if (cs != null) {
    ...
    private CustomerServiceRequest parseCustomerService(String line) {
        String[] p = line.split("\\|");
        if (p.length < 7) return null;
        try {
            return new CustomerServiceRequest(
    ...
    private void persistCustomerServices() {
        List<String> lines = new ArrayList<>();
        for (CustomerServiceRequest cs : customerServices) lines.add(cs.toFileString());
    ...
    /** Customer requests a service alongside a reservation */
    public CustomerServiceRequest requestService(int customerId, int reservationId, int serviceId, int quantity) {
        Service s = getServiceById(serviceId);
        if (s == null || !s.isAvailable()) return null;
        double total = s.calculatePrice(quantity);
        CustomerServiceRequest cs = new CustomerServiceRequest(nextCustomerServiceId++, customerId,
    ...
    public List<CustomerServiceRequest> getCustomerServicesByReservation(int reservationId) {
        List<CustomerServiceRequest> list = new ArrayList<>();
        for (CustomerServiceRequest cs : customerServices)
            if (cs.getReservationId() == reservationId) list.add(cs);
        return list;
    }

    public List<CustomerServiceRequest> getAllCustomerServices() {
